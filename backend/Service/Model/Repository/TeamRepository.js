        ...
        teamByHash(teamid,hash).then(res1=>{
            ...
            userrepo.getUserPermission('IsAdmin',userid).then(async res=>{
                ...
                getTeamMembers(teamid).then(async result=>{   
                    console.log(result)                         
                    resolve((jsonParser.combineJSON(protocol.status(true),UserDaO.GetUserListJson(result.map(element=>jsonParser.combineJSON(userrepo.ViewToUser(element),{"Tag":element['Tag']?element['Tag']:""}))))));
                }).catch((e)=>{
                    logger.error(e);
                    resolve((jsonParser.combineJSON(protocol.status(false),protocol.error(99))));
                })
